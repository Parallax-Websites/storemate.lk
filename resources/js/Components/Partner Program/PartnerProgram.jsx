        ...
        <div ref={componentRef} className="relative bg-gray-900 overflow-hidden">
            ...
            <div className="absolute inset-0">
                <img
                    ...
                    src="https://lightcyan-buffalo-783093.hostingersite.com/wp-content/uploads/2025/09/cta_bg_3-1.jpg"
                ...
                <div className={`max-w-4xl transition-all duration-1000 ${isVisible ? 'translate-y-0 opacity-100' : 'translate-y-8 opacity-0'}`}>
                    ...
                    <div className="mb-8">
                        <span className="inline-flex items-center px-6 py-3 rounded-full text-sm font-bold tracking-widest bg-white/10 backdrop-blur-sm border border-white/20 text-white">
                            🤝 {tPartnerProgram('partnerProgram.hero.badge')}
                        </span>
                    </div>

                    {/* Heading */}
                    <h1 className="text-4xl font-extrabold tracking-tight text-white sm:text-5xl lg:text-6xl mb-8">
                        <span className="text-white">{tPartnerProgram('partnerProgram.hero.title.part1')}</span>
                        <span className="block" style={{color: '#006daf'}}>{tPartnerProgram('partnerProgram.hero.title.part2')}</span>
                    </h1>

                    {/* Description */}
                    <p className="text-xl text-gray-300 mb-12 leading-relaxed max-w-3xl">
                        {tPartnerProgram('partnerProgram.hero.description')}
                    </p>
                    ...
                    <div className="flex flex-col sm:flex-row gap-6 mb-16">
                        ...
                        >
                            {tPartnerProgram('partnerProgram.hero.buttons.joinProgram')}
                        ...
                        >
                            {tPartnerProgram('partnerProgram.hero.buttons.learnMore')}
                        </button>
                    </div>

                    {/* Partner Types */}
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        {tPartnerProgram('partnerProgram.hero.partnerTypes').map((partnerType, index) => (
                            <div key={index} className="bg-white/10 backdrop-blur-sm rounded-xl p-6 border border-white/20 transform hover:scale-105 transition-all duration-300">
                                ...
                                <h3 className="text-lg font-semibold text-white mb-2">{partnerType.title}</h3>
                                <p className="text-gray-300 text-sm">{partnerType.description}</p>
                            </div>
                        ))}
                    </div>
                </div>
