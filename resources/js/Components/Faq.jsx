...
const Faq = () => {
    ...
    // Define FAQ data directly in component
    const faqData = [
        {
            question: {
    ...
    const [faq, setFaq] = useState(
        faqData.map((item, index) => ({
            question: item.question[currentLanguage] || item.question.en,
            answer: item.answer[currentLanguage] || item.answer.en,
            open: false
        }))
    );

    // Update FAQ when language changes
    useEffect(() => {
        setFaq(faqData.map((item, index) => ({
            question: item.question[currentLanguage] || item.question.en,
            answer: item.answer[currentLanguage] || item.answer.en,
            open: false
        })));
